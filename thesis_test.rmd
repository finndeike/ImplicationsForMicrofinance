...
maturity_data <- stata_data %>% filter(onetermshown == 1, low==1 | med ==1)
...
ggplot(stata_data, aes(offer4, lastamount) + geom_point() + geom_abline()
...
# ggplot(stata_data) + aes(x=offer4, y=tookup_outside_only) + geom_point()
...
rel_data <- stata_data
rel_data$province <- relevel(as.factor(rel_data$province), ref=4)
rel_data$branchuse <- relevel(as.factor(rel_data$branchuse), ref=4)
...
stargazer(reg4_1, reg4_2, reg4_3, omit = c("offer4"), type="html", header = FALSE)
# * add controls
...
reg4_4 <- lm(loansize ~ factor(offer4) + low + med + waved2 + waved3 + factor(sales_grossincome) + factor(sales_grossincomesq) + factor(sales_netincome) + factor(sales_netincomesq) + appscore + appscoresq + appscore0 + itcscore + itczero + itcscoresq + dormancy + trcount + female + dependants + married + age + agesq + rural + edhi + province + branchuse, data = filter(stata_data, offer4==final4, normrate_less==1, tookup==1))
...
reg4_5 <- lm(loansize ~ offer4 + low + med + waved2 + waved3 + sales_grossincome + sales_grossincomesq + sales_netincome + sales_netincomesq + appscore + appscoresq + appscore0 + itcscore + itczero + itcscoresq + dormancy + trcount + female + dependants + married + age + agesq + rural + edhi + as.factor(province), data = filter(stata_data, offer4==final4, normrate_less==1, tookup==1))
...
reg4_8 <- lm(lnloansize ~ lnoffer4 + low + med + waved2 + waved3 + sales_grossincome + sales_grossincomesq + sales_netincome + sales_netincomesq + appscore + appscore0 + appscoresq + itcscore + itczero + itcscoresq + dormancy + trcount + female + dependants + married + age + agesq + rural + edhi + province, data = filter(stata_data, offer4==final4, tookup==1, normrate_less==1, grossincome!=0), right=+Inf, left=-Inf)
...
ZAF <- getData("GADM", country = "ZAF", level = 1)
...
ZAF@data <- merge(ZAF@data, df, by.x = c("NAME_1"), by.y = c("province"))

borders <- list("sp.polygons", ZAF, fill = "transparent", col = 'black', first = FALSE)
spplot(ZAF, "interest_rate", main = "South Africa Demographic", sub = "Average Interest Rate per Borrower", cuts = 6, sp.layout=borders) 
...
zaf_map <- tmap_last()
tmap_save(zaf_map, "zaf_map.html")

```

- read_dta() -    https://stat.ethz.ch/R-manual/R-devel/library/foreign/html/read.dta.html
