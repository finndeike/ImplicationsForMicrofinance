...
df <- stata_data %>% group_by(province) %>% summarise(offer4 = mean(offer4))
...
map <- left_join(df, ZAF@data, by=c("province" = "NAME_1"), as.sf = TRUE)

karte <- st_as_sf(map)
map1 <- map %>% ggplot(aes(fill = offer4)) + geom_sf()
