...
`read_dta` from the package `haven` reads a file in Stata version 5-12 binary format into a data frame in R. This function only supports Stata formats after 12, but since we have a 5-12 format it can be used. We are able to load in the package with the `library(.package)` command and the data with the `read_dta(.data)` command.
