...
setwd("~/Documents/GitHub/ImplicationsForMicrofinance/")

ps.name = "ImplicationsForMicrofinance"
sol.file = paste0(ps.name, "_sol.Rmd")
libs = c("ggplot2", "haven", "statar", "dplyr", "weights", "lmtest", "lfe", "stargazer", "regtools", "tidyverse", "readr", "knitr", "qwraps2", "AER", "VGAM", "censReg","sf", "ggplot2", "tmap", "tmaptools", "leaflet", "dplyr", "gridExtra")
...
Lack of access to financial services is therefore one of the largest constraints to private sector development in Africa. Addressing this shortfall requires creating new institutions and building operational and managerial capacity from the ground up (Earne, J. et al., 2014).
...
The problem set consists of various exercises, each exercise covers one or more topics. In the first exercise we will get to know the experiment and have a look at our data set. This includes information of the specific loan offers and client characteristics. The next section will test if randomly assigned mail offer letters are indeed randomized using different statistical tests. In the fourth section we will estimate price elasticities for consumer credit in respect to price and loan size and explain the general model. In the last section we will examine the optimal short-run pricing strategy for the lender by determining the costs of reducing and increasing the loan price.  
...
In this section we will explain the experiment and get to know the data we use in this interactive R-Problemset.

### Experimental Design

The lender Karlan and Zinma cooperated with operated for more than 20 years as one of the largest and most-profitable microlenders in South Africa. He competed in a cash loan industry segment that offer small, high-interest, short-term uncollateralized credit fixed monthly repayment schedules to the working poor population.
...
The maturity is also randomized and the process is orthogonal to the offer rate randomization. But the difference is that only low- and medium-risk clients received the suggestion randomization. Those who were eligible for the suggestion randomization or in other words maturities longer than four month, received a randomized sample maturity of six or twelve month. High-risk clients were not able to choose higher maturities than 4 months. The only value which is presented on the offer letter and is not randomized is the loan size. It is equal to the last loan size of each client.
The experiment was carried out in three mailer waves of start dates grouped by different branches geographically. First, a pilot test in three branches in July 2003, and than the experiment was expanded to the remaining 83 branches divided into two more mailer waves in September 2003 and October 2003. 

Each offer contains a deadline between two to six weeks. An offer was accepted by a client by entering a branch office and filling out the application with an loan officer. The loan applications are evaluated per the lenders standard procedure independent of the experimental rates. Following the estimation of the loan officer the clients are assigned a proportional loan size and maturity.
...
The main data set of the experiment is named **_kz_demandelasts_aer08.dta_**. The ending `.dta` indicates that it is a Stata file. The data set contains data of only the clients who actually received an offer letter from the lender. In fact 1,358 mailers were returned to the lender by the postal service and 3,000 contained atypical relationships between the offer rate and maturity. This leaves a data set of 53,810 clients. The information of the clients range from experimental variables to demographic characteristics.
Before we can take a look at our data set **_kz_demandelasts_aer08.dta_**, we have to read in the Stata data file with the R-function `read_dta()` from the `haven` package. However, we must load the package at first to be able to use the function.
...
#< hint
display("Just fill in the name of the data set 'dat' in the first place holder and the command 'read_dta()' in the second")
#>
```
...
![Fig. 2  | JJ Allaire, retrieved from https://www.youtube.com/channel/UC8wovsufF42uACug27bO3wQ))](images/Award Pictures/jj_allaire.png)
#>
...
1.6) Replace the question marks with the missing values and functions to create a boxplot of distribution of the offer rate for each risk category. If you think your answer is right press the `Check` button.
```{r "1 d) 1"}
#< fill_in
dat$risk <- factor(dat$risk, levels=c("LOW", "MEDIUM", "HIGH"))
dat %>%
  ggplot(aes(x=risk, y=___)) +
  ___ +
  stat_summary(fun = mean, geom = "point", size=3, col="red", shape=18)
#>
dat %>%
  ggplot(aes(x=risk, y=offer4)) +
  geom_boxplot() +
  stat_summary(fun = mean, geom = "point", size=3, col="red", shape=18)
```

The box plot shows us the distribution of the offer rate conditional on the risk category. The average offer rate value is displayed by the red diamonds. For clients who are classified as low risk the average offer rate lies at about 6 percent, for medium risk at slightly more then 7 percent and for those who are classified as high risk at about 8.5 percent. The median marks the mid-point of the data and is shown by the line that divides the box into two parts. Half the scores are greater than or equal to this value and half are less. We can see that the median offer rate for low risk clients are approximately at 6 percent per month for clients of the medium risk category at about 8 and for high risk clients at slightly above 9. The middle box represents the middle 50% of scores for the group. The range of scores from lower to upper quartile is referred to as the inter-quartile range. The middle 50% of scores fall within the inter-quartile range. Seventy-five percent of the scores fall below the upper quartile and twenty-five percent of scores fall below the lower quartile. Furthermore, the upper and lower whiskers represent scores outside the middle 50%. Any value that is higher or lower than the upper limit or lesser than the lower limit will be the outliers, those are displayed by the black points. The outliers show us that the offer rates are the most compressed distributed for low risk clients, slightly less for medium risk clients and most widely distributed for high risk clients.

These determinations are vague estimates of the graphic. But we can have a more detailed look at the distribution by using simple mathematical operations conditional on the risk category.
...
To determine the values for all three risk categories, we have to perform the operations on each client risk group. To do so we can use the function `group_by()` and `summarise()` of the `dplyr` package.
...
values <- dat %>%
  group_by(___) %>%
  summarise(min_value=___(offer4), max_value=___(offer4))
...
display("values <- dat %>%
        group_by(???) %>%
        summarise(min_value=min(offer4), max_value=???(offer4))")
#>
```
...
```{r "1 g) 1", output="htmlwidget", widget="leaflet"}
#< task_notest
...
![Fig. 3  | Bartolomeu Dias, retrieved from https://escola.britannica.com.br/artigo/Bartolomeu-Dias/481145))](images/Award Pictures/bartolomeu_dias.png)
#>


## Exercise 3 -- Randomization Process Validation

In the last section we loaded in the data file, got a rough look at our data and created a interactive map of South Africa. Furthermore, we explained the experiment.
Hence, in this part we will check if the offer rate randomization process was successful using several statistical tests. There could be various reasons due to which randomization process may fail or may not have been proper. For example, other factors could have an effect on the dependent variable in the sense of being sufficient conditions for causal influence on the dependent variable. Furthermore, the validation prevents the selection bias and insures against the accidental bias.
...
```{r "2 a) 5"}
#< fill_in
reg2_1 <- ___(___ ~ dormancy + lntrcount + female + dependants + married + lnage + rural + edhi + itcscore_100 + itczero + appscore_100 + low + med + waved2 + waved3, data=dat)
...
#< info "p-value"
The **p-value** is the smallest significance level which leads to a rejection of the null hypothesis $H_0$. If the actual significance level $\alpha$ is therefore lower then the **p-value** $p-value > \alpha$, the null hypothesis can not be rejected.  

Check out E.Spodarev (2021) for further information.
#>

In fact the p-value of all observable variables is significantly higher than the significance level. Therefore the null hypothesis $H_0$ can not be rejected. Which indicates that indeed the offer rate is highly likely independent of the other observable characteristics and thus the randomization process was successful 

We can also portray the effect that different explanatory variables have on the dependent variable in a regression model with the function `effectplot()` of the `regtools` package. The basic concept of this plot is to compare the effects of the explanatory variables if they change from their 10 percent to their 90 percent quantile or for binary variable the effect of changing from 1 to 0. To create this kind of plot, we just have to add our regression into the function braces `effectplot(.reg)` (For further information checkout: https://github.com/skranz/regtools/blob/master/man/effectplot.Rd).
...
### The Offer and Loan Application Process

#### Intertemporal Substitution

Since we just learned, that it is reasonable to assume that the randomization process was successful, we will now check if intertemporal substitution is a problem. Clients could have borrowed after the deadline to the lender standard rates. For this we will use a **probit regression** which is very similar to simple linear regression. The difference is that a probit regression is a binomial regression which means the outcome is either a success (1) or a failure (0). Where in linear regressions the outcome is scale or rather numerical.

#< info "Probit Regression with glm()"
The `glm()`function from the `stats`-package is used to fit generalized linear models. It is similar structure to the `lm()`regression, with the exception that with the argument *family* we can specify what kind of linear model we want to use. In our case we want to use a probit link function. than the function looks as follows: *glm(<FORMULA>, family=binomial(link="probit"), <DATA>)*
...
The model of a probit regression will be explained more in detail in exercise 3.
...
As we can see the p-value is 0.851 which is considerably higher than the significance level. Thus, the null hypothesis can not be rejected. Which substantiates that offer rates at or below the standard rate did not influence the take-up after the deadline significantly. This seems conclusive since the clients borrowed at the standard rate schedule after the deadline.

We can also observe the relationship between the take-up after the deadline and the offer rate from a graphically. Therefore, we will use the `ggplot()` function again to declare the input data frame, than we will utilize the `stat_smooth()` (will be explained later in detail) to plot our probit regression `reg2_2`.
...
ggplot(dat,aes(x=offer4 + low + med + waved2 + waved3, y=tookup_afterdead_enforced)) + 
  stat_smooth(method='glm',family=binomial(link='probit'), level=0.95) +
  ylim(min=0, max=1) +
...
The graph corroborates our thesis since there is no clear tendency that the probability of a take-up after the deadline increases or decreases with a higher offer rate. Moreover, the 95% confidence interval indicates a very small deviaton of the values from the median. 

#### Rejection Decision

In addition we want to find out if the rejection decisions of the clients were correlated with the offer rate awarding process. The approach is the same as the previous regression. However this time we have to reduce the data frame to only clients who applied for a loan. Furthermore we will add a new function called `stargazer()` to get a nicer output table which holds the regression results of the two regressions.
...
2.2.5) Execute the Regression `reg2_3` than use the function `stargazer()` to create a nicer-looking output table of the two regressions. We want to set `type="html"` and `header=FALSE`. If you need a little hint, press `hint`.
...
stargazer(reg2_2, reg2_3, type="html", header=FALSE)
#< hint
display("stargazer(reg2_2, reg2_???, type=???, header=???)")
#>
```

We see that the p-value is once again substantially higher than the significance level. Therefore, the null hypothesis $H_0$ can not be rejected for both regressions, which corroborates that the rejection decision was also not influenced by the offer rate.
...
![Fig. 4  | Andrien-Marie Legendre, retrieved from https://commons.wikimedia.org/wiki/File:Adrien-Marie_LEGENDRE,_mathématicien_français_du_XIXème_siècle.jpg))](images/Award Pictures/am_legendre.jpg)
#>
...
success: Your answer is correct, it makes no sense to have a probability lower than zero or higher than 1!
...
![Fig. 5  | Gustave Theodor Fechner, retrieved from https://vlp.mpiwg-berlin.mpg.de/people/data?id=per68))](images/Award Pictures/g_fechner.jpg)
#>
...
#< hint
display("reg3_1 <- glm(??? ~ offer4 + low + med + waved2 + waved3, family = binomial(link = ???), data=filter(dat, ???))
summary(???)")
#>
```
...
```{r "4.1.2 b)", error=FALSE, results='asis'}
#< task_notest
...
```{r "4.1.2 c)", error=FALSE, results='asis'}
#< task_notest
...
#< quiz "Price Elastcity of Loan Demand"
question: Calculate the price elasticity of demand for our sub-sample in regression `reg3_1`. The mean offer rate is 8 and the mean of clients who applied is 0.085. Type in you answer in the box below rounded by two digits.
answer: -0.28
success: Your answer is correct, a 1 % increase in the offer rate reduces take-up by 0.28 %!
failure: Your answer is wrong, take another look at the formula above!
#>
...
```{r "4.1.3 a)", error=FALSE, results='asis'}
#< task_notest
...
#< quiz "Borrowed Elsewhere"
question: Did higher offer rates induce more borrowing from other financial institutions?.
sc:
...
- There is no significant relationship between higher offer rates and borrowing from an outside lender.*
...
success: Your answer is correct!
failure: Your answer is wrong, take a look at the significance levels!
#>
...
![Fig. 6  | Daniel Kahneman, retrieved from https://de.pokerstrategy.com/news/world-of-poker/GTO-Pokertheorie:-Die-Prospect-Theory_104328/))](images/Award Pictures/d_kahneman.jpg)
#>
...
summary(reg4_1)
#< hint
display("reg4_1 <- felm(???~ offer4 |low + ??? + ??? + ??? |0| branchuse,  data = filter(dat, offer4==final4, normrate_less==1))")
#>
```
...
```{r "4.2 b)", warning=FALSE, results='asis'}
#< task_notest
...
```{r "4.2 c) 1", warning=FALSE, results='asis'}
#< task_notest
...
```{r "4.2 c) 2", warning=FALSE, results='asis'}
...
```{r "4.2 d)", warning=FALSE, results='asis'}
#< fill_in
...
![Fig. 7  | Alfred Marshall, retrieved from http://www.hagen-bobzin.de/mikro/Marshall.html))](images/Award Pictures/a_marshall.jpeg)
#>
...
```{r "5.1.2)", warning=FALSE, results='asis'}
#< task_notest
...
#< quiz "Demand Curve Revenue"
question: The gross revenue result implies that a 100-basis-point increase of the offer rate increase the gross revenue by R???.
answer: 2.6
success: Your answer is correct, the gross revenue result implies that a 100-basis-point increase in the offer rate increases gross revenue by R2.6!
failure: Your answer is wrong, take another look a the regression estimate in 5.1.2)!
#>
...
```{r "5.1.3)", warning=FALSE, results='asis'}
#< task_notest
...
```{r "5.1.4)", warning=FALSE, results='asis'}
#< task_notest
...
#< quiz "Cut offer rate?"
question: What do you think, would a lower offer rate increase the lenders revenue significantly?
sc:
- Yes.
- No.*
success: Your answer is correct, for the explanation check out below!
failure: Your answer is wrong!
#>
...
![Fig. 8  | Muhammed Yunus, https://www.expert-marketplace.de/redner/muhammed-yunus-friedensnobelpreistraeger/))](images/Award Pictures/m_yunus.jpg)
#>
...
- Spodarev. E. (2021): "Mathematical Statistics", Ulm University
...
- Joe Cheng, Bhaskar Karambelkar and Yihui Xie (2021). leaflet: Create Interactive Web Maps
  with the JavaScript 'Leaflet' Library. R package version 2.0.4.1.
  https://CRAN.R-project.org/package=leaflet.
- Peter DeWitt (2020). qwraps2: Quick Wraps 2. R package version 0.5.0.
  https://CRAN.R-project.org/package=qwraps2.
- S. Gaure. lfe: Linear group fixed effects. R package version 2.8-6, 2021.
- Matthieu Gomez (2020). statar: Tools Inspired by 'Stata' to Manipulate Tabular Data. R
  package version 0.7.3. https://CRAN.R-project.org/package=statar.
- Arne Henningsen (2020). censReg: Censored Regression (Tobit) Models. R package version
  0.5-32. https://CRAN.R-project.org/package=censReg.
- Marek Hlavac (2018). stargazer: Well-Formatted Regression and Summary Statistics Tables.
  R package version 5.2.1. https://CRAN.R-project.org/package=stargazer.
- Christian Kleiber and Achim Zeileis (2008). Applied Econometrics with R. New York:
  Springer-Verlag. ISBN 978-0-387-77316-2. URL https://CRAN.R-project.org/package=AER.
- Sebastian Kranz (2018). regtools: Some tools for regressions and presentation of
  regressions results. R package version 2018.11.20.
- Sebastian Kranz (2020). RTutor: Interactive R problem sets with automatic testing of
  solutions and automatic hints. R package version 2020.11.25.
- Josh Pasek, with some assistance from Alex Tahk, some code modified from R-core;
  Additional contributions by Gene Culter and Marcus Schwemmle. (2020). weights: Weighting
  and Weighted Statistics. R package version 1.0.1.
  https://CRAN.R-project.org/package=weights.
- E. Pebesma, 2018. Simple Features for R: Standardized Support for Spatial Vector Data.
  The R Journal 10 (1), 439-446, https://doi.org/10.32614/RJ-2018-009.
- Martijn Tennekes (2018). “tmap: Thematic Maps in R.”Journal of Statistical Software, 84(6),
  1-39. doi: 10.18637/jss.v084.i06 (URL: https://doi.org/10.18637/jss.v084.i06).
- Martijn Tennekes (2021). tmaptools: Thematic Map Tools. R package version 3.1-1.
...
- Hadley Wickham, Romain François, Lionel Henry and Kirill Müller (2020). dplyr: A Grammar
  of Data Manipulation. R package version 1.0.2. https://CRAN.R-project.org/package=dplyr.
...
- Thomas W. Yee (2020). VGAM: Vector Generalized Linear and Additive Models. R package
  version 1.1-4. URL https://CRAN.R-project.org/package=VGAM.
- Achim Zeileis, Torsten Hothorn (2002). lmtest: Diagnostic Checking in Regression Relationships. R News 2(3), 7-10. URL https://CRAN.R-project.org/doc/Rnews/.


### Websites

- https://www.bbc.com/news/world-africa-14094918, retrieved 28. January 2021
- https://cran.r-project.org/web/packages/stargazer/stargazer.pdf, retrieved 20. February 2021
- https://datacarpentry.org/R-ecology-lesson/04-visualization-ggplot2.html, retrieved 2. February 2021
- https://dplyr.tidyverse.org/reference/filter.html, retrieved 23. January
- https://dplyr.tidyverse.org/reference/group_by.html, retrieved 26. January 2021
- https://dplyr.tidyverse.org/reference/summarise.html, retrieved 26. January 2021
- https://dplyr.tidyverse.org/reference/mutate.html, retrieved 27. January 2021
- https://www.econlib.org/library/Enc/bios/Marshall.html, retrieved 3. February 2021
- https://escola.britannica.com.br/artigo/Bartolomeu-Dias/481145, retrieved 15. April 2021
- https://www.expert-marketplace.de/redner/muhammed-yunus-friedensnobelpreistraeger/, retrieved 20. March 2021
- https://github.com/skranz/regtools/blob/master/man/effectplot.Rd, retrieved 18. February 2021
- South African Government (2021): South Africa's provinces, Retrieved from             https://www.gov.za/about-sa/south-africas-provinces#
- http://www.hagen-bobzin.de/mikro/Marshall.html, retrieved 20. March 2021
- https://www.investopedia.com/terms/p/p-value.asp, retrieved 12. February 2021
- https://microfinancingafrica.org/10-profound-quotes-about-microfinance/, retrieved 19. January 2021
- https://de.pokerstrategy.com/news/world-of-poker/GTO-Pokertheorie:-Die-Prospect-Theory_104328/, retrieved 20. March 2021
- https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/lm, retrieved 01. February 2021
- https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/summary, retrieved 01. February 2021
- https://rdrr.io/github/skranz/regtools/src/R/showreg.r, retrieved 12. March 2021
- https://rdrr.io/cran/lfe/man/felm.html, retrieved 18. March 2021
...
- https://stat.ethz.ch/R-manual/R-devel/library/foreign/html/read.dta.html, retrieved 22. January 2021
- https://commons.wikimedia.org/wiki/File:Adrien-Marie_LEGENDRE,_mathématicien_français_du_XIXème_siècle.jpg
- https://en.wikipedia.org/wiki/Gustav_Fechner, retrieved  12. March 2021
- https://www.xe.com/currencyconverter/convert/?Amount=1&From=ZAR&To=USD, retrieved 30. March 2021
- https://www.youtube.com/channel/UC8wovsufF42uACug27bO3wQ, retrieved 15. April 2021
