...
Good luck and have fun solving this interactive problem set to increase your knowledge about microfinance or rather microcredit in South Africa!
...
Muhammad Yunus is a Bangladeshi social entrepreneur, banker, economist, and civil society leader. He is one of the pioneers of microcredit and microfinance and the founder of the "Grameen Bank" which is also known as "The Bank of the Poor". 

**Zitat aus Buch raussuchen**
...
#< info "ggplot()"
The `ggplot()` function from the `ggplot2` package is a simple way to convert data in a data frame into complex plots.  
#>
...
The Probit regression will be explained more in detal in exercise 3.
...
We can illustrate the demand curve for column (2) 
4.1.4)
```{r "4.1.2 c)", error=FALSE}
...
In this second section of exercise 4 we want to examine the price sensitivity of loan size. Our analysis in this part is dependent on three conditional traits and controls: *conditional on borrowing*, *branch fixed effects* and *additional controls for demos and credit risk*.
...
```{r "4.2 c)", warning=FALSE, results='asis'}
#< task_notest
