...
Therefore we will replicate the testing of Sean S. Karlan and Jonathan Zinma on the hypotheses of price inelastic demand using randomized trials conducted by a high-risk consumer lender in South Africa. The field experiment is based on randomized individual interest rate and maturity direct mail offers to more than 50,000 former clients of the lender based on the client's prior rate.
...
1.9) Group the data frame `borrowed` by the `province` the applicant applied in using the function `group_by()`. The new data frame should be named `zaf`. Then use the function `summarise()` to apply the given operations for each group. Afterwards show the new data frame `provinces`. 
```{r "1 e) 1"}
#< fill_in
borrowed <- dat %>% filter(tookup == 1)
...
#< task_notest
ggplot(dat,aes(x=offer4,y=tookup_afterdead_enforced))+ 
...
#< hint
display("stargazer(reg2_1, ???, ???, type=???, header=???)")
#>
```
...
#< task_notest
reg3_2 <- glm(applied ~ offer4 + low + med, family = binomial(link = "probit"), data=filter(dat, normrate_less == 0))
...
```{r "4.1.2 c) 1", error=FALSE}

x_plot1 <- lm(offer4 ~ low + med + waved2 + waved3, data=dat)
y_plot1 <- lm(tookup ~ low + med + waved2 + waved3, data=dat)

ggplot(dat, aes(x=x_plot1$residuals, y=y_plot1$residuals)) + geom_point()
...
ggplot(dat, aes(x=x_plot1$residuals, y=y_plot1$residuals)) + geom_point()
#>
ggplot(dat, aes(x=x_plot1$residuals, y=y_plot1$residuals)) + geom_smooth(method="loess", se=FALSE, span=0.5)
...
reg3_5 <- glm(tookup_outside_only ~ offer4 + low + med, family = binomial(link = "probit"), data = filter(dat, normrate_less == 0))
...
success: Your answer is correct!
failure: Your answer is wrong, take a look at the signficance levels!
#>
...
reg3_8 <- glm(??? ~ normrate_more + low + med + waved2 + waved3, family = binomial(link = "probit"), data = dat)

reg3_9 <- glm(??? ~ offer4 + low + med, family = binomial(link = "probit"), data = filter(dat, normrate_less == 0))
...
reg3_8 <- glm(tookup_afterdead_enforced ~ normrate_more + low + med + waved2 + waved3, family = binomial(link = "probit"), data = dat)

reg3_9 <- glm(tookup_afterdead_enforced ~ offer4 + low + med, family = binomial(link = "probit"), data = filter(dat, normrate_less == 0))
...
question: Calculate the price elasticity of demand for our sub-sample. The mean offer rate is 7.8 and the mean loan size 106. Type in you answer in the box below rounded by two digits.
answer: -0.32
success: Your answer is correct!
...
## Exercise 5.1 -- Profit Maximation
...
```{r "5.1.2 a)", warning=FALSE, results='asis'}
#< task_notest
...
```{r "5.1.2 b)", warning=FALSE, results='asis'}
#< task_notest
...
question: The gross revenue result implies that a 100-basis-point decrease of the offer rate reduces gross revenue by R???.
answer: 2.6
success: Your answer is correct, the gross revenue result implies that a 100-basis-point decrease of the offer rate reduces gross revenue by R2.6!
failure: Your answer is wrong, take another look a the regression estimate in 5.1.2)!
#>

```{r "5.1.2 c)", warning=FALSE, results='asis'}
#< task_notest
reg5_2 <- felm(pstdue_average ~ offer4 | low + med + waved2 + waved3 |0| branchuse, data = filter(dat, offer4==final4, normrate_less==1))

reg5_3 <- censReg(pstdue_average ~ offer4 + low + med + waved2 + waved3, data = filter(dat, offer4==final4, normrate_less==1, tookup==1))
...
reg6_1 <- lm(loansize ~ offer4 + low + med + waved2 + waved3 + grossincome + grossincomesq + dormancy + trcount + female + dependants + married + age + agesq + rural + edhi + appscore + appscoresq + appscore0 + itcscore + itcscoresq + itczero + province + branchuse, data = filter(dat, offer4==final4, normrate_less==1))
# edhi=1 -> High education

reg6_2 <- lm(loansize ~ offer4 + low + med + waved2 + waved3 + grossincome + grossincomesq + dormancy + trcount + female + dependants + married + age + agesq + rural + appscore + appscoresq + appscore0 + itcscore + itcscoresq + itczero + province + branchuse, data = filter(dat, offer4==final4, normrate_less==1, edhi==1))

# dormancy<10 -> Borrowed in last 9 months

reg6_3 <- lm(loansize ~ offer4 + low + med + waved2 + waved3 + grossincome + grossincomesq + trcount + female + dependants + married + age + agesq + rural + edhi + appscore + appscoresq + appscore0 + itcscore + itcscoresq + itczero + province + branchuse, data = filter(dat, offer4==final4, normrate_less==1, dormancy<10))

reg6_4 <- lm(loansize ~ offer4+ low + med + waved2 + waved3 + grossincome + grossincomesq + dormancy + female + dependants + married + age + agesq + rural + edhi + appscore + appscoresq + appscore0 + itcscore + itcscoresq + itczero + province + branchuse, data = filter(dat, offer4==final4, normrate_less==1, trcount>2))
...
```{r "5.2.1 a)", warning=FALSE, results='asis'}
#< task_notest
dat <- dat %>% mutate(grossincomecat2 = xtile(grossincome, n=2))
dat <- dat %>% mutate(sales_grossincomecat2 = xtile(grossincome, n=10))
...
reg7_1 <- lm(applied ~ offer4 + low + med + waved2 + waved3, data=filter(dat, female==1, normrate_less==1))
...
reg7_2 <- lm(applied ~ offer4 + low + med + waved2 + waved3, data=filter(dat, normrate_less==1, grossincomecat2==1))
...
reg7_3 <- lm(applied ~ offer4 + low + med + waved2 + waved3, data=filter(dat, normrate_less==1, grossincomecat2==1, female==1))
...
reg7_4 <- lm(loansize ~ offer4 + low + med + waved2 + waved3, data=filter(dat, normrate_less==1, offer4==final4, female==1))
...
reg7_5 <- lm(loansize ~ offer4 + low + med + waved2 + waved3, data=filter(dat, normrate_less==1, offer4==final4, grossincomecat2==1))
...
reg7_6 <- lm(loansize ~ offer4 + low + med + waved2 + waved3, data=filter(dat, normrate_less==1, offer4==final4, grossincomecat2==1, female==1))
...
reg7_7 <- lm(loansize ~ offer4 + low + med + waved2 + waved3, data=filter(dat, tookup==1, normrate_less==1, offer4==final4, female==1))
...
reg7_8 <- lm(loansize ~ offer4 + low + med + waved2 + waved3, data=filter(dat, sales_grossincomecat2==1, normrate_less==1, offer4==final4, tookup==1))
...
reg7_9 <- lm(loansize ~ offer4 + low + med + waved2 + waved3, data=filter(dat, sales_grossincomecat2==1, normrate_less==1, offer4==final4, female==1, tookup==1))
